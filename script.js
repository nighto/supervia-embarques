// SuperVia Customers data
const SUPERVIA_CUSTOMERS_API = 'supervia_customers.json'
...
axios
    ...
    .then(res => {
        ...
        // get data, then
        axios.get(SUPERVIA_CUSTOMERS_API).then(res => {
            ...
            let customer_stations = res.data.stations

            console.log('customer_stations', customer_stations)

            // for each SuperVia message
            customer_stations.map(customer_station => {
                // find the station
                let station = stations.features.find( station => returnStationById(station, customer_station.id) )

                if (station) {
                    // record the message as a station property
                    station.properties.values = customer_station.values
                }
            })
            ...
            stations.features.map(station => {
                // if it has messages
                if (station.properties.values) {
                    // find its coords
                    let stationCoords = [station.geometry.coordinates[1], station.geometry.coordinates[0]]

                    // create its text with last message
                    let text = '' + station.properties.values

                    // add a marker
                    let marker = L.marker(stationCoords).bindPopup(text).addTo(map)
                }
            })
        })
    })
